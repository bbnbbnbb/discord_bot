...
async def on_ready():
    ...
    await bot.change_presence(status=discord.Status.online, activity=discord.Game("노래 연주 준비"))
...
async def 들어와(ctx):
    try:
        ...
        try:
            ...
            await ctx.send("음성 채널에 유저가 접속해있지 않아요.")
...
async def 나가(ctx):
    try:
        ...
        await ctx.send("이미 음성 채널에 들어와 있지 않아요.")
...
async def play(ctx, *, msg):
    ...
    if not vc.is_playing():
        ...
        result, URLTEST = title(msg)
        song_queue.append(URLTEST)
        await ctx.send("이미 노래가 재생중이어서" + result + "을(를) 대기열에 추가했어요.")
...
async def stop(ctx):
    if vc.is_playing():
        ...
        await ctx.send("지금은 노래가 재생되고 있지 않아요.")
...
\np도움말 -> 뮤직봇의 모든 명령어를 볼 수 있습니다.
\np들어와 -> 뮤직봇을 자신이 속한 채널로 부릅니다.
\np나가 -> 뮤직봇을 자신이 속한 채널에서 내보냅니다.
\npURL [노래링크] -> 유튜브URL를 입력하면 뮤직봇이 노래를 틀어줍니다.
(목록재생에서는 사용할 수 없습니다.)
\npplay [노래이름] -> 뮤직봇이 노래를 검색해 틀어줍니다.
\npoff -> 현재 재생중인 노래를 끕니다.
pstop -> 현재 재생중인 노래를 일시정지시킵니다.
presume -> 일시정지시킨 노래를 다시 재생합니다.
\np지금노래 -> 지금 재생되고 있는 노래의 제목을 알려줍니다.
\np멜론차트 -> 최신 멜론차트를 재생합니다.
\np즐겨찾기 -> 자신의 즐겨찾기 리스트를 보여줍니다.
p즐겨찾기추가 [노래이름] -> 뮤직봇이 노래를 검색해 즐겨찾기에 추가합니다.
p즐겨찾기삭제 [숫자] ->자신의 즐겨찾기에서 숫자에 해당하는 노래를 지웁니다.
\np목록 -> 이어서 재생할 노래목록을 보여줍니다.
p목록재생 -> 목록에 추가된 노래를 재생합니다.
